...
int main(int argc, char *argv[])
{
    ...
    while (runTime.run())
    {
        ...
        (
                        ...
                        mesh,
                        dimensionedScalar("ch", dimless, 0.0)
        );
        ...
        volScalarField cd("cd", sqrt((-(runTime.deltaTValue())*sqr(ch))/log(Cr)));
        ...
        // --- Solve pB
        tmp<fvScalarMatrix> pBEqn
        (
            fvm::ddt(pB) 
            +
            sqr(ch)*fvc::div(bf)
            ==
            fvm::SuSp(sqr(ch/cd)/pB,pB)
        );

//        solve(fvm::ddt(pB) + fvc::div(phipB) == fvm::SuSp(sqr(ch/cd)/pB,pB));         //adicionado: solução da equação potêncial
