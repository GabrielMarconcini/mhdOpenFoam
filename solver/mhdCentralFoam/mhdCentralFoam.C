...
int main(int argc, char *argv[])
{
    argList::addNote
    (
        "Density-based MHD compressible flow solver based on central-upwind"
        " schemes of Kurganov and Tadmor, for MHD compressible flows."
    );
    ...
    while (runTime.run())
    {
        ...
        surfaceScalarField ch("ch", CFL/(dt*max(1/h))));                      //adicionado: constante cd
        surfaceScalarField cd("cd", sqrt((-dt*sqr(ch))/ln(Cr));               //adicionado: constante ch
        ...
        phiB = aphiv_pos*B_pos + aphiv_neg*B_neg;                //adicionado: (a*phif+*B+) + ((1-a)*phif-*B-) + wf*(B- + B+)
